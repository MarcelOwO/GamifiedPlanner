...
class UserRepositoryImpl(
    ...
    override fun observeUser(): Flow<User?> =
        callbackFlow {
            ...
            val listener =
                getUserDoc(uid!!).addSnapshotListener { snapshot, error ->
                    if (error != null) {
                        close(error)
    ...
    override fun observeAchievementsProgress(): Flow<Map<String,Long> =
        callbackFlow {
            ...
            val listener =
                getAchievementsColl(uid!!).addSnapshotListener { snapshot, error ->
                    if (error != null) {
                        close(error)
                        return@addSnapshotListener
                    }
    ...
    override fun observeInventory(): Flow<List<UserInventoryItem>> =
        callbackFlow {
            if (uid == null) {
                trySend(emptyList())
                return@callbackFlow
            ...
            val listener =
                getInventoryColl(uid!!).addSnapshotListener { snapshot, error ->
                    if (error != null) {
                        close(error)
                        return@addSnapshotListener
                    }
    ...
    // refactor later to usecase if I  can be bothered
    ...
    override fun observeNotifications(): Flow<Boolean> =
        callbackFlow {
            if (uid == null) {
                trySend(false)
                return@callbackFlow
