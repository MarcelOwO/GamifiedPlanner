...
class AddBalanceUseCase(
    ...
    suspend operator fun invoke(amount:Long): PlannerResult<Unit> {
        ...
        if (userId == null) {
            logger.e("Invoking add currency usecase requires user to be logged in")
        ...
        val newStats =
            currentStats.copy(
                currency = currentStats.currency + amount
            )

        userRepo.updateStats(userId, newStats)

        logger.i("Invoking observe add usecase was successful")
